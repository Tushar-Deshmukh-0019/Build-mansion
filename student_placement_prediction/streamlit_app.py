...
def send_otp_sms(to_number, otp):
    ...
    try:
        ...
        client.messages.create(
            body=f"Your Joblib OTP is: {otp}. Valid for 5 minutes. Do not share.",
...
# 5. SESSION STATE
# ==========================================
for k, v in {"logged_in":False,"otp_sent":False,"otp":"","user_id":"",
             "otp_via_sms":False,"sms_error":""}.items():
...
if not st.session_state.logged_in:
    ...
    with mid:
        st.markdown("<h2 style='text-align:center;color:white;'>🔐 Secure Student Login</h2>",
                    unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        with st.container(border=True):
            mobile = st.text_input("📱 Mobile Number", placeholder="Enter 10-digit number",
                                   disabled=st.session_state.otp_sent, max_chars=10)
            if not st.session_state.otp_sent:
                if st.button("Send OTP", use_container_width=True):
                    if len(mobile) == 10 and mobile.isdigit():
                        ...
                        ok, msg = send_otp_sms(mobile, otp_code)
                        st.session_state.otp_via_sms = ok
                        st.session_state.sms_error   = msg
                        st.rerun()
                    else:
                        st.error("Please enter a valid 10-digit mobile number.")
            else:
                if st.session_state.otp_via_sms:
                    st.success(f"✅ OTP sent to +91-{st.session_state.user_id} via SMS")
                else:
                    err = st.session_state.sms_error
                    if err == "TWILIO_NOT_CONFIGURED":
                        st.warning(f"⚠️ Demo Mode — OTP: **{st.session_state.otp}**")
                    else:
                        st.warning(f"⚠️ SMS failed. Demo OTP: **{st.session_state.otp}**")
                entered_otp = st.text_input("🔑 Enter OTP", max_chars=6, placeholder="6-digit OTP")
                c1, c2 = st.columns(2)
                if c1.button("✅ Verify & Login", use_container_width=True):
                ...
                if c2.button("✏️ Edit Number", use_container_width=True):
                    st.session_state.otp_sent    = False
                    st.session_state.otp_via_sms = False
                    st.rerun()
    st.stop()
...
if st.sidebar.button("🚪 Logout"):
    for k in ["logged_in","otp_sent","otp","user_id","otp_via_sms"]:
