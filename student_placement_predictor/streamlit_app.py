                    ...
                    with col_r:
                        if st.button("🔄 Resend OTP", use_container_width=True, key="btn_resend_otp"):
                            with st.spinner("Resending..."):
                                email_to_resend = st.session_state.get("otp_email", otp_email)
                                d = db.api_send_otp(email_to_resend)
                                if "error" in d:
                                    st.error(f"❌ {d['error']}")
                                else:
                                    # Update session to reflect new OTP sent
                                    st.session_state.otp_sent = True
                                    st.session_state.otp_email = email_to_resend
...
page = option_menu(
    menu_title=None,
    options=["Home","Career Tools","AI Mentor","Messages","Live Dashboard","Student Help","Admin Portal","About"],
    icons=["house","tools","robot","envelope","bar-chart-line","question-circle","shield-lock","info-circle"],
...
# ==========================================
    ...
    if 'career_gender' not in st.session_state:
        st.session_state.career_gender = "Male"
    if 'career_stream' not in st.session_state:
        st.session_state.career_stream = "Computer Science"
    if 'career_cgpa' not in st.session_state:
        st.session_state.career_cgpa = 7.5
    if 'career_num_backlogs' not in st.session_state:
        st.session_state.career_num_backlogs = 0
    if 'career_internships' not in st.session_state:
        st.session_state.career_internships = 1
    if 'career_projects' not in st.session_state:
        st.session_state.career_projects = 2
    if 'career_hackathons' not in st.session_state:
        st.session_state.career_hackathons = 1
    if 'career_project_domain' not in st.session_state:
        st.session_state.career_project_domain = "Web Development"
    ...
    with l_col:
        st.markdown('<div class="section-label">📚 Academic Profile</div>',
                    unsafe_allow_html=True)
        with st.container(border=True):
            cgpa = st.slider("CGPA", 0.0, 10.0, st.session_state.career_cgpa, step=0.1,
                             help="Your current cumulative GPA out of 10",
                             key="cgpa_slider")
            st.session_state.career_cgpa = cgpa
            
            col_a, col_b = st.columns(2)
            with col_a:
                age = st.number_input("Age", 18, 30, st.session_state.career_age, key="age_input")
                st.session_state.career_age = age
                
                gender = st.selectbox("Gender", ["Male", "Female"], 
                                     index=0 if st.session_state.career_gender == "Male" else 1,
                                     key="gender_select")
                st.session_state.career_gender = gender
            with col_b:
                stream_options = [
                ...
                stream_idx = stream_options.index(st.session_state.career_stream) if st.session_state.career_stream in stream_options else 0
                stream = st.selectbox("Stream", stream_options, index=stream_idx, key="stream_select")
                st.session_state.career_stream = stream
                
                num_backlogs = st.number_input(
                    "Number of Active Backlogs", 
                    min_value=0, max_value=5, value=st.session_state.career_num_backlogs,
                    help="Total number of current active backlogs (0-5)",
                    key="backlogs_input"
                )
                st.session_state.career_num_backlogs = num_backlogs

        st.markdown('<div class="section-label" style="margin-top:16px;">💼 Experience & Activities</div>',
                    unsafe_allow_html=True)
        with st.container(border=True):
            internships = st.select_slider(
                "Internships Completed", options=[0,1,2,3,4,5], value=st.session_state.career_internships,
                help="Number of internships done so far",
                key="internships_slider"
            )
            st.session_state.career_internships = internships
            
            col_c, col_d = st.columns(2)
            with col_c:
                projects = st.number_input(
                    "Projects Built", 0, 20, st.session_state.career_projects,
                    help="Total personal/academic projects (GitHub, college, freelance)",
                    key="projects_input"
                )
                st.session_state.career_projects = projects
            with col_d:
                hackathons = st.number_input(
                    "Hackathons Participated", 0, 20, st.session_state.career_hackathons,
                    help="Number of hackathons you have participated in",
                    key="hackathons_input"
                )
                st.session_state.career_hackathons = hackathons
        
        # Project Domain Selection
        st.markdown('<div class="section-label" style="margin-top:16px;">🎨 Project Domain</div>',
                    unsafe_allow_html=True)
        with st.container(border=True):
            # Domain options based on stream
            domain_options = {
                "Computer Science": ["Web Development", "Mobile App Development", "Machine Learning", 
                                    "Data Science", "Cloud Computing", "Cybersecurity", "DevOps", "Blockchain"],
                "Information Technology": ["Web Development", "Mobile App Development", "Database Management", 
                                          "Network Security", "Cloud Computing", "System Administration"],
                "Mechanical": ["CAD Design", "Robotics", "Thermal Systems", "Manufacturing", 
                              "Automotive Engineering", "3D Printing"],
                "Civil": ["Structural Design", "Construction Management", "Urban Planning", 
                         "Environmental Engineering", "Transportation Engineering"],
                "Electrical": ["Power Systems", "Embedded Systems", "Renewable Energy", 
                              "Control Systems", "Electronics Design"],
                "Electronics And Communication": ["IoT", "Signal Processing", "VLSI Design", 
                                                 "Embedded Systems", "Telecommunications", "Robotics"]
            }
            
            available_domains = domain_options.get(stream, ["General"])
            # Update domain if stream changed and current domain not available
            if st.session_state.career_project_domain not in available_domains:
                st.session_state.career_project_domain = available_domains[0]
            
            domain_idx = available_domains.index(st.session_state.career_project_domain) if st.session_state.career_project_domain in available_domains else 0
            project_domain = st.selectbox(
                "Primary Project Domain",
                options=available_domains,
                index=domain_idx,
                help="Select the main domain of your projects",
                key="domain_select"
            )
            st.session_state.career_project_domain = project_domain
        ...
        # Backlog penalty: -3 points per backlog
        preview_score += max(10 - (num_backlogs * 3), 0)
            ...
            with st.spinner("Analysing your profile..."):
                try:
                    d = db.api_predict(
                        ...
                        cgpa           = float(cgpa),
                        backlog        = 1 if num_backlogs > 0 else 0,  # For compatibility
                        num_backlogs   = int(num_backlogs),
                    ...
                    if "error" in d:
                        ...
                        # Job Recommendations
                        st.markdown("<br>", unsafe_allow_html=True)
                        st.markdown('<div class="section-label">💼 Job Recommendations</div>',
                                    unsafe_allow_html=True)
                        ...
                        try:
                            ...
                            st.warning(f"Could not load job recommendations: {e}")
                except Exception as e:
...
# ==========================================
    ...
    st.markdown("<h2 style='color:#eef0f8;margin-bottom:4px;'>📬 Admin Messages</h2>", unsafe_allow_html=True)
    st.markdown("<p style='color:#9aa3bf;margin-bottom:24px;'>View guidance and feedback from your admin</p>", unsafe_allow_html=True)
    
    try:
        # Get notifications where admin has replied to this student
        conn = get_connection()
        cursor = conn.cursor()
        
        cursor.execute("""
            SELECT admin_reply, replied_at, message, status, confidence, created_at
        ...
        if not messages:
            st.info("📭 No messages from admin yet. Complete a prediction to receive personalized guidance!")
        else:
            st.success(f"✉️ You have {len(messages)} message(s) from admin")
            
            for msg in messages:
                admin_reply, replied_at, original_msg, status, confidence, created_at = msg
                ...
                with st.container(border=True):
                    ...
                    with col2:
                        st.markdown(f"<span style='color:{status_color};font-weight:600;'>{confidence*100:.1f}%</span>", unsafe_allow_html=True)
                    
                    # Original prediction info
                    st.markdown(f"<p style='color:#9aa3bf;font-size:12px;margin-top:4px;'>Prediction Date: {created_at}</p>", unsafe_allow_html=True)
                    st.markdown(f"<p style='color:#cbd5e1;font-size:13px;font-style:italic;margin-top:8px;'>{original_msg}</p>", unsafe_allow_html=True)
                    
                    st.markdown("---")
                    
                    # Admin reply
                    st.markdown(f"""
                        <div style='background:rgba(124,92,252,0.1);border-left:3px solid #7c5cfc;
                            border-radius:10px;padding:14px 18px;margin-top:12px;'>
                            <div style='color:#a78bfa;font-size:11px;font-weight:700;margin-bottom:8px;text-transform:uppercase;'>
                                💬 Admin Reply • {replied_at}
                            </div>
                            <p style='color:#eef0f8;font-size:14px;line-height:1.6;margin:0;'>{admin_reply}</p>
                        </div>
                    """, unsafe_allow_html=True)
...
# ==========================================
    ...
    ADMIN_EMAIL = "tushartushar74303@gmail.com"
    
    # Check if user is admin OR if dashboard is unlocked in session
    is_admin = st.session_state.user_email == ADMIN_EMAIL
    dashboard_unlocked = st.session_state.get("dashboard_unlocked", False)
...
# ==========================================
    ...
    # Tabs for admin
    tab_queries, tab_notifications, tab_guidance, tab_stats = st.tabs([
        "📬 Support Queries", "🔔 Student Notifications", "💬 Send Guidance", "📊 Platform Stats"
    ])
    ...
    # New Tab: Student Notifications
    with tab_notifications:
        st.markdown('<div class="section-label">Student Prediction Notifications</div>', unsafe_allow_html=True)
        
        try:
            notif_data = db.api_get_admin_notifications(ADMIN_KEY)
            
            if "error" in notif_data:
                st.error(f"❌ {notif_data['error']}")
            else:
                notifications = notif_data.get("notifications", [])
                
                # Filter by status
                filter_notif_status = st.selectbox("Filter by Status", 
                    ["All", "high_confidence", "needs_guidance", "moderate"], key="notif_filter")
                
                if filter_notif_status != "All":
                    notifications = [n for n in notifications if n["status"] == filter_notif_status]
                
                # Stats
                high_conf = sum(1 for n in notif_data.get("notifications", []) if n["status"] == "high_confidence")
                needs_help = sum(1 for n in notif_data.get("notifications", []) if n["status"] == "needs_guidance")
                moderate = sum(1 for n in notif_data.get("notifications", []) if n["status"] == "moderate")
                
                n1, n2, n3 = st.columns(3)
                n1.metric("🎉 High Confidence", high_conf)
                n2.metric("⚠️ Needs Guidance", needs_help)
                n3.metric("📊 Moderate", moderate)
                ...
                if not notifications:
                    ...
                    for notif in notifications:
                        status_color = "#22c55e" if notif["status"] == "high_confidence" else "#f87171" if notif["status"] == "needs_guidance" else "#f59e0b"
                        status_icon = "🎉" if notif["status"] == "high_confidence" else "⚠️" if notif["status"] == "needs_guidance" else "📊"
                        
                        with st.container(border=True):
                            col_n1, col_n2 = st.columns([3, 1])
                            with col_n1:
                                st.markdown(f"**{status_icon} {notif['user_name']}** ({notif['stream']})")
                            with col_n2:
                                st.markdown(f"<span style='color:{status_color};font-weight:600;'>{notif['confidence']*100:.1f}%</span>", unsafe_allow_html=True)
                            
                            st.markdown(f"📧 {notif['user_email']} | 🎯 {notif['project_domain']}")
                            st.markdown(f"📊 CGPA: {notif['cgpa']} | 📅 {notif['created_at']}")
                            st.markdown(f"<p style='color:#9aa3bf;font-size:13px;margin-top:8px;'>{notif['message']}</p>", unsafe_allow_html=True)
                            
                            # Check if already replied
                            admin_reply = notif.get('admin_reply')
                            replied_at = notif.get('replied_at')
                            
                            if admin_reply:
                                st.markdown(f"""
                                    <div style='background:rgba(34,197,94,0.1);border-left:3px solid #22c55e;
                                        border-radius:8px;padding:10px 14px;margin-top:10px;'>
                                        <div style='color:#22c55e;font-size:11px;font-weight:700;margin-bottom:4px;'>
                                            ✅ REPLIED ON {replied_at}
                                        </div>
                                        <p style='color:#cbd5e1;font-size:13px;margin:0;'>{admin_reply}</p>
                                    </div>
                                """, unsafe_allow_html=True)
                            else:
                                # Reply form
                                with st.expander(f"💬 Reply to {notif['user_name']}", expanded=False):
                                    reply_msg = st.text_area(
                                        "Your Reply",
                                        placeholder="Write your guidance message here...",
                                        height=120,
                                        key=f"reply_{notif['id']}"
                                    )
                                    
                                    if st.button(f"📤 Send Reply", key=f"send_reply_{notif['id']}", use_container_width=True):
                                        if not reply_msg.strip():
                                            st.error("❌ Reply cannot be empty")
                                        else:
                                            result = db.api_reply_notification(ADMIN_KEY, notif['id'], reply_msg.strip())
                                            if "error" in result:
                                                st.error(f"❌ {result['error']}")
                                            else:
                                                st.success(f"✅ Reply sent to {notif['user_name']}!")
                                                st.rerun()
        
        except Exception as e:
            st.error(f"❌ Error loading notifications: {e}")
    
    # New Tab: Send Guidance
    with tab_guidance:
        st.markdown('<div class="section-label">Send Guidance to Students</div>', unsafe_allow_html=True)
        
        with st.container(border=True):
            # Check if student was selected from notifications
            selected_student = None
            for key in st.session_state.keys():
                if key.startswith("selected_student_"):
                    selected_student = st.session_state[key]
                    break
            
            if selected_student:
                st.info(f"📝 Composing message for: **{selected_student['user_name']}** ({selected_student['user_email']})")
                student_id = selected_student["user_id"]
                student_name = selected_student["user_name"]
                student_email = selected_student["user_email"]
            else:
                st.markdown("### Select Student")
                # Get all users for dropdown
                try:
                    conn = get_connection()
                    users_df = pd.read_sql("SELECT id, name, email FROM users ORDER BY name", conn)
                    conn.close()
                    ...
                    user_options = [f"{row['name']} ({row['email']})" for _, row in users_df.iterrows()]
                    selected_user = st.selectbox("Choose Student", user_options)
                    
                    # Extract user details
                    selected_idx = user_options.index(selected_user)
                    student_id = str(users_df.iloc[selected_idx]["id"])
                    student_name = users_df.iloc[selected_idx]["name"]
                    student_email = users_df.iloc[selected_idx]["email"]
            ...
            guidance_type = st.selectbox("Message Type", [
                "improvement", "congratulations", "general", "urgent"
            ])
            
            guidance_message = st.text_area(
                "Your Message",
                placeholder="Write your guidance message here...",
                height=150,
                key="guidance_msg_input"
            )
            
            col_send, col_clear = st.columns(2)
            
            with col_send:
                if st.button("📤 Send Guidance", use_container_width=True):
                    if not guidance_message.strip():
                        st.error("❌ Message cannot be empty")
                    else:
                        result = db.api_send_admin_guidance(
                            ADMIN_KEY,
                            student_id,
                            student_name,
                            student_email,
                        ...
                        if "error" in result:
                            ...
                            st.success(f"✅ Guidance sent to {student_name}!")
        ...
        st.markdown("---")
        st.markdown("### 📜 Recent Guidance Sent")
        
        try:
            conn = get_connection()
            recent_guidance = pd.read_sql("""
                SELECT user_name, user_email, admin_message, guidance_type, created_at, read_by_student
                FROM admin_guidance
            ...
            if recent_guidance.empty:
                st.info("No guidance messages sent yet")
            else:
                for _, row in recent_guidance.iterrows():
                    read_status = "✅ Read" if row["read_by_student"] else "📭 Unread"
                    st.markdown(f"""
                        <div style='background:rgba(255,255,255,0.03);border-left:3px solid #7c5cfc;
                            padding:12px;border-radius:8px;margin-bottom:10px;'>
                            <div style='display:flex;justify-content:space-between;margin-bottom:6px;'>
                                <strong style='color:#eef0f8;'>{row['user_name']}</strong>
                                <span style='color:#9aa3bf;font-size:12px;'>{read_status}</span>
                            </div>
                            <p style='color:#9aa3bf;font-size:12px;margin:4px 0;'>📧 {row['user_email']} | 🏷️ {row['guidance_type']}</p>
                            <p style='color:#cbd5e1;font-size:13px;margin:8px 0;'>{row['admin_message'][:100]}...</p>
                            <p style='color:#5a6380;font-size:11px;margin:4px 0;'>📅 {row['created_at']}</p>
                        </div>
                    """, unsafe_allow_html=True)
        
        except Exception as e:
            st.error(f"Error loading recent guidance: {e}")
