...
for k, v in {"logged_in": False, "user_id": "", "user_name": "",
             ...
             "otp_via": "", "reg_done": False, "admin_authenticated": False,
             "reg_otp_sent": False}.items():
...
if st.sidebar.button("🚪 Logout"):
    for k in ["logged_in","user_id","user_name","user_email","otp_sent","otp_demo","otp_via"]:
...
if page == "Home":
    ...
    st.caption(f"🔌 DB: `{_db_url[:80]}`")
    if st.button("🧪 Test DB Write Now"):
        import psycopg2 as _pg
        try:
            _conn = _pg.connect(_db_url, connect_timeout=10)
            _c = _conn.cursor()
            _c.execute("SELECT COUNT(*) FROM predictions")
            _before = _c.fetchone()[0]
            _c.execute("""INSERT INTO predictions (user_id,age,gender,stream,internships,cgpa,backlog,hostel,projects,hackathons,result,confidence,timestamp)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
                ("dbtest",21,"Male","Computer Science",1,7.5,0,0,2,1,1,0.85,"2026-01-01 00:00:00"))
            _conn.commit()
            _c.execute("SELECT COUNT(*) FROM predictions")
            _after = _c.fetchone()[0]
            _c.execute("DELETE FROM predictions WHERE user_id='dbtest'")
            _conn.commit()
            _conn.close()
            st.success(f"✅ Write works! Before={_before} After={_after}")
        except Exception as _ex:
            st.error(f"❌ EXACT ERROR: {_ex}")
