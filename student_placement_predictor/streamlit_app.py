...
# ==========================================
    ...
    try:
        conn = get_connection()
        hist_df = pd.read_sql(
            f"SELECT * FROM predictions WHERE user_id='{st.session_state.user_id}' ORDER BY id DESC",
            conn
        )
        conn.close()
    ...
    with tab_my_queries:
        ...
        try:
            ...
            user_queries_df = pd.read_sql(
                f"SELECT * FROM support_queries WHERE user_id = '{st.session_state.user_id}' ORDER BY id DESC",
                conn
            )
            conn.close()
