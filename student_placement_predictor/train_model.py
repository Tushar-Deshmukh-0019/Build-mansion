...
n_synth = 600  # counter-examples to inject

# High CGPA but NOT placed (poor internships / backlogs)
high_cgpa_not_placed = pd.DataFrame({
    "Age":              rng.integers(20, 25, n_synth // 2),
    "Gender":           rng.integers(0, 2, n_synth // 2),
    "Stream":           rng.integers(0, len(le_stream.classes_), n_synth // 2),
    "Internships":      rng.integers(0, 1, n_synth // 2),          # 0 internships
    "CGPA":             rng.uniform(7.5, 10.0, n_synth // 2),      # high CGPA
    "Hostel":           rng.integers(0, 2, n_synth // 2),
    "HistoryOfBacklogs": rng.integers(0, 2, n_synth // 2),         # some have backlogs
    "PlacedOrNot":      0,                                          # NOT placed
})

# Low CGPA but placed (strong internships, no backlogs)
low_cgpa_placed = pd.DataFrame({
    "Age":              rng.integers(20, 25, n_synth // 2),
    "Gender":           rng.integers(0, 2, n_synth // 2),
    "Stream":           rng.integers(0, len(le_stream.classes_), n_synth // 2),
    "Internships":      rng.integers(2, 4, n_synth // 2),          # 2-3 internships
    "CGPA":             rng.uniform(5.5, 7.0, n_synth // 2),       # lower CGPA
    "Hostel":           rng.integers(0, 2, n_synth // 2),
    "HistoryOfBacklogs": np.zeros(n_synth // 2, dtype=int),        # no backlogs
    "PlacedOrNot":      1,                                          # placed
})

df = pd.concat([df, high_cgpa_not_placed, low_cgpa_placed], ignore_index=True)
