...
).strip()  # strip accidental whitespace from env var

def get_db():
    """Return a fresh psycopg2 connection with keepalive to prevent SSL drops."""
    url = DATABASE_URL
    # Neon requires SSL; add it if not already present
...
# Module-level connection (re-created on error)
_conn = None
_cur  = None

def _get_cursor():
    """Return (conn, cursor), reconnecting if the connection dropped."""
    global _conn, _cur
    try:
...
def init_db():
    ...
    # Migrate old column names if they exist (user_email → user_id, placement_result → result, etc.)
    try:
        c.execute("""
            DO $$
            BEGIN
                IF EXISTS (
                    ...
                    WHERE table_name='predictions' AND column_name='user_email'
                ) THEN
                    ALTER TABLE predictions RENAME COLUMN user_email TO user_id;
                END IF;
                IF EXISTS (
                    SELECT 1 FROM information_schema.columns
                    WHERE table_name='predictions' AND column_name='placement_result'
                ) THEN
                    ALTER TABLE predictions RENAME COLUMN placement_result TO result;
                END IF;
                IF EXISTS (
                    SELECT 1 FROM information_schema.columns
                    WHERE table_name='predictions' AND column_name='readiness_score'
                ) THEN
                    ALTER TABLE predictions RENAME COLUMN readiness_score TO confidence;
                END IF;
                IF EXISTS (
                    SELECT 1 FROM information_schema.columns
                    WHERE table_name='predictions' AND column_name='predicted_at'
                ) THEN
                    ALTER TABLE predictions RENAME COLUMN predicted_at TO timestamp;
                END IF;
            END $$;
        """)
    except Exception:
...
# ─────────────────────────────────────────
# ML MODEL — load, retrain if incompatible
# ─────────────────────────────────────────
def _load_or_retrain():
    """Load model files. If numpy version mismatch, retrain on the spot."""
...
def api_login(email, password):
    """Verify email + password. Returns user info or error."""
    email = email.strip().lower()
...
def _send_email_otp(to_email, otp):
    """Send OTP via Resend API (HTTPS port 443 — works on HF Spaces free tier)."""
...
def api_send_otp(email):
    """Generate OTP, store it, send via Resend API. Returns dict."""
    email = email.strip().lower()
...
def api_verify_otp(email, otp):
    """Verify OTP and return user info. Returns dict."""
    email = email.strip().lower()
    otp   = otp.strip()
    try:
        conn, c = _get_cursor()
...
def api_predict(user_id, age, gender, stream, internships, cgpa, backlog,
    ...
    """Run ML prediction, save to DB, return result dict."""
    try:
